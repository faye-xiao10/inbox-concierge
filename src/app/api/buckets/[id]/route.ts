...
export async function DELETE(
  ...
  try {
    const displaced = await db
      .select({ id: classifications.id, threadId: classifications.threadId })
    ...
    await db.update(classifications)
      .set({ bucketId: null, classificationTier: null, confidence: null })
      .where(and(eq(classifications.userId, session.userId), eq(classifications.bucketId, bucketId)));
    ...
    return new Response(JSON.stringify({ displaced: displaced.length, displacedThreadIds: displaced.map((d) => d.threadId) }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
