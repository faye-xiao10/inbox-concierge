...
function Card({ value, label, subtext, pulse }: any) {
  return (
    <div style={{
      ...
      borderRadius: 12,
      padding: '24px 20px', // Increased padding for better scale
      display: 'flex',
      flexDirection: 'column',
      minHeight: '140px' // Ensure cards don't shrink too much
    }}>
      <div className={pulse ? 'animate-pulse' : ''} style={{
        fontSize: typeof value === 'string' ? '2.5rem' : 'inherit', // Larger font
        fontWeight: 800,
  ...
  return (
    <div style={{ marginTop: 48, paddingBottom: 60 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
        ...
        <Card value={metrics ? `$${metrics.estimatedCost.toFixed(3)}` : ''} label="Estimated Cost" subtext="Based on actual tokens" pulse={pulse} />
      ...
      {/* High-Contrast Methodology Legend */}
      <div style={{ 
        marginTop: 32, 
        padding: '24px', 
        ...
        <div style={{ 
          ...
          <MethodologyItem code="T0" title="Gmail Native" desc="Uses existing Google system labels and metadata." />
          <MethodologyItem code="T1" title="Domain Rules" desc="Matches trusted sender domains with 100% accuracy." />
          <MethodologyItem code="T2" title="Vector Memory" desc="Uses pgvector similarity search to match previous user actions." />
          <MethodologyItem code="T3" title="Claude 3.5" desc="Deep reasoning fallback for complex or unseen email types." />
        </div>
      </div>
    </div>
  );
